import { Box, Button, Container, Flex, HStack, Image, Input, Stack, Text, useToast } from "@chakra-ui/react"
...
function CreateUserProfile() {
  ...
  const onCreateProfile = async (data: IInputs) => {
    ...
    await HttpClient<ICreateClientResponse>(HttpMethod.POST, "/api/c/me", payload, options)
      .then((response) => {
        toast({
      ...
      <Flex gap={"6"}>
          ...
          <Box width={"2xl"}>
            <form onSubmit={handleSubmit(onCreateProfile)}>
              ...
              <HStack gap={"2"} mt={"16"}>
                <Box w={"full"}>
                  ...
                  {errors.firstName && <Text 
                    color={"red.400"} 
                    fontSize={"smaller"} 
                    fontWeight={"normal"}>{errors.firstName.message}</Text>}
                </Box>
                <Box w={"full"}>
                  <Input
                    type={"text"}
                  ...
                  {errors.lastName && <Text 
                    color={"red.400"} 
                    fontSize={"smaller"} 
                    fontWeight={"normal"}>{errors.lastName.message}</Text>}
                </Box>
              </HStack>
              <Input
                type={"date"}
                ...
                {errors.birthDate && <Text 
                    color={"red.400"} 
                    fontSize={"smaller"} 
                    fontWeight={"normal"}>{errors.birthDate.message}</Text>}
              <Input
                type={"text"}
              ...
              {errors.presentAddress && <Text 
                  color={"red.400"} 
                  fontSize={"smaller"} 
                  fontWeight={"normal"}>{errors.presentAddress.message}</Text>}
              <Input
                type={"text"}
                ...
                {errors.permanentAddress && <Text 
                    color={"red.400"} 
                    fontSize={"smaller"} 
                    fontWeight={"normal"}>{errors.permanentAddress.message}</Text>}
