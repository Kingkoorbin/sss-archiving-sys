...
function AdminEmployeePreview() {
    ...
    const onSubmitWorkExperience: SubmitHandler<IExperiencePayload> = async (data) => {
    ...
    const onDeleteWorkExperience = async (id: number) => {
        try {
            await axios.delete(
                `${API_BASE_URL}/api/client/v1/${id}/workhistory`,
                {
                    headers: {
                        Authorization: `Bearer ${getAuthResponse?.access_token}`,
                    },
                ...
                <div>
                    ...
                    <div>
                        <Switch
                            ...
                            defaultChecked />
                        <div style={{ background: "white", padding: 50, width: 800, borderRadius: 20, display: state.toggleWorkExperienceSection ? "block" : "none" }}>
                            <form onSubmit={handleSubmitExperienceFormData(onSubmitWorkExperience)}>
                            ...
                            {state.workExperience?.map((el) => {
                                return <div key={el.id} >
                                    <Card bordered={false} style={{ marginTop: 20, width: 800 }}>
                                        <Flex justify='space-between' align='center'>
                                            <p style={{ fontSize: 24, padding: 0, margin: 0 }}>{el.company_name}</p>
                                            <Tooltip title="Delete">
                                                <CloseOutlined style={{ cursor: "pointer", color: "red" }} onClick={() => onDeleteWorkExperience(el.id)} />
                                            </Tooltip>
                                        </Flex>
                                        <p style={{ fontSize: 16, padding: 0, margin: 0, fontWeight: "600" }}>{el.position}</p>
                                        <p style={{ fontSize: 12, padding: 0, margin: 0, color: "#888" }}>{moment(el.start_date).format("YYYY, MMM DD")} - {moment(el.end_date).format("YYYY, MMM DD")} </p>
