...
function PreviewRecords() {
    ...
    const handleGetTransactions = async (
        ...
        ).get<any, IGetClientTransactionsParams>(
            ...
            (item: ITransaction) => {
                ...
                return {
                    ...
                    "item-amount": currency.format(
                        item.content.reduce(
                            (total: any, contentItem: any) =>
                                total + contentItem.amount,
                            0,
                        ),
                    ),
                    "item-details": (
        ...
        setState((prev) => ({
            ...prev,
            txnRevenue:
                Number(getTransactionsResp.headers["swsya-txn-revenue"]) || 0,
            stlCount:
                Number(getTransactionsResp.headers["swsya-stl-count"]) || 0,
            swtCount:
                Number(getTransactionsResp.headers["swsya-swt-count"]) || 0,
            txnCount:
                Number(getTransactionsResp.headers["swsya-txn-count"]) || 0,
            txnTotal:
                Number(getTransactionsResp.headers["swsya-txn-total"]) || 0,
    ...
    const handleVerifyToken = async (): Promise<boolean> => {
        ...
        if (getAuthResponse!.token) {
            const verifyToken = await SwsyaClient.post<any, any>(
                API.verifyToken,
                {
    ...
    const handleChangeDate: DatePickerProps["onChange"] = async (
        _,
        dateString: any,
    ) => {
        setState((prev) => ({
    ...
    const handleChangeTime: TimePickerProps["onChange"] = async (
        _,
        time: any,
    ) => {
        setState((prev) => ({
    ...
    const handleGetTransactionData = async () => {
        const getTransactionDataResp = await SwsyaClient.setAuthToken(
            getAuthResponse!.token.data,
        ).get<any, any>(API.myTransactionData, {
            user: location?.state?.client?.user,
        });
        setState((prev) => ({
            ...prev,
            chartLabels: getTransactionDataResp.data.map((v: any) => {
                return dayjs(v.schedule).format("MMM DD");
            }),
