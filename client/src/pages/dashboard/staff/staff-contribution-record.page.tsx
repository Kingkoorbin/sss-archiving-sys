...
import { Alert, Button, Flex, Modal, Table, Tooltip, Upload, message } from 'antd';
...
export default function StaffContributionRecord() {
    ...
    const onGetUserProfile = async () => {
        ...
        const getProfileResponse: AxiosResponse = await axios.get(`${API_BASE_URL}/api/user/v1`, {
            ...config,
        });
    ...
    const getContributions = async (data?: IGeneratePdfPayload) => {
        ...
        try {
            ...
            const getAllContributionsResponse: AxiosResponse & { data: IContribution[] } = await axios.get(`${API_BASE_URL}/api/record/v1`, {
                params: data,
                ...config,
            });
            ...
            setState((prev) => ({
                ...prev,
                isFetchingContributions: false,
                contributions: getAllContributionsResponse.data?.map((el: IContribution) => ({
                    ...el,
                    key: el.id,
                    actions: <Flex gap={10}>
                        <Tooltip title={hasPermission(user.user_permissions, TPermissionTypes.EDIT) ? "Edit" : "No Permission"}>
                            <Button
                                type='dashed'
                                icon={<EditOutlined />}
                                disabled={!hasPermission(user.user_permissions, TPermissionTypes.EDIT)}
                                onClick={() =>
                                    setState((prev) => ({
                                        ...prev,
                        ...
                        <Tooltip title={hasPermission(user.user_permissions, TPermissionTypes.DELETE) ? "Delete" : "No Permission"}>
                            <Button
                                style={{ color: "red" }}
                                icon={<CloseOutlined />}
                                disabled={!hasPermission(user.user_permissions, TPermissionTypes.DELETE)}
    ...
    return (
        <>
            ...
            <div style={{ padding: 50 }}>
                <Tooltip title={hasPermission(state.user?.user_permissions!, TPermissionTypes.GENERATE) ? "Generate" : "No Permission"}>
                    <Dragger disabled={!hasPermission(state.user?.user_permissions!, TPermissionTypes.GENERATE)} {...props}>
                            ...
                            <div style={{ marginLeft: 20 }}>
                                <Tooltip title={hasPermission(state.user?.user_permissions!, TPermissionTypes.GENERATE) ? "Generate" : "No Permission"}>
                                    <Button
                                        ...
                                        disabled={state.contributions.length >= 100 || !hasPermission(state.user?.user_permissions!, TPermissionTypes.GENERATE)}
                                    >
                                        Generate
                                    </Button>
                                </Tooltip>
